...
// There are some dependent inequalities here, but better safe than sorry.
TEST_CASE("ThickMaze neighbours should obey expected rules") {
    constexpr auto width = 25;
    constexpr auto height = 20;
    const types::Dimensions2D dim{width, height};
    thickmaze::ThickMazeGeneratorByHomomorphism dfs{maze::DFSMazeGenerator{dim}};
    const auto tm = dfs.generate();

    SECTION("All walls should be considered out of bounds, and have no neighbours") {
        for (auto x = 0; x < tm.getWidth(); ++x)
            for (auto y = 0; y < tm.getHeight(); ++y)
                if (tm.cellIs(x, y) == thickmaze::CellType::WALL) {
                    const types::Cell c = types::cell(x, y);
                    REQUIRE_FALSE(tm.cellInBounds(c));
                    REQUIRE(tm.neighbours(c).empty());
                }
    }
    SECTION("A cell not generated by a cellular automaton that has no neighbours must be a wall") {
        for (auto x = 0; x < tm.getWidth(); ++x)
            for (auto y = 0; y < tm.getHeight(); ++y) {
                const types::Cell c = types::cell(x, y);
                if (tm.neighbours(c).empty())
                    REQUIRE(tm.cellIs(c) == thickmaze::CellType::WALL);
            }
    }
    SECTION("Every cell has between 0 and 4 neighbours") {
        for (auto x = 0; x < tm.getWidth(); ++x)
            for (auto y = 0; y < tm.getHeight(); ++y) {
                const types::Cell c = types::cell(x, y);
                REQUIRE(tm.neighbours(c).size() >= 0);
                REQUIRE(tm.neighbours(c).size() <= 4);
            }
    }
    SECTION("Every cell has between 0 and 4 walls") {
        for (auto x = 0; x < tm.getWidth(); ++x)
            for (auto y = 0; y < tm.getHeight(); ++y) {
                const types::Cell c = types::cell(x, y);
                REQUIRE(tm.numCellWalls(c) >= 0);
                REQUIRE(tm.numCellWalls(c) <= 4);
            }
    }
    SECTION("For a given cell, the number of neighbours and the number of walls should sum to four") {
        for (auto x = 0; x < tm.getWidth(); ++x)
            for (auto y = 0; y < tm.getHeight(); ++y) {
                const types::Cell c = types::cell(x, y);
                const auto numNbrs = tm.neighbours(c).size();
                const auto numWalls = tm.numCellWalls(c);
