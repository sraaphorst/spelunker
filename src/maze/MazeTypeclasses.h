...
namespace spelunker::typeclasses {
    ...
    struct Homomorphism<maze::Maze, thickmaze::ThickMaze> {
        static const thickmaze::ThickMaze morph(const maze::Maze &m) {
            ...
            auto contents = thickmaze::createThickMazeLayout(twidth, theight);
            ...
            for (auto x = 0; x < mwidth; ++x)
                for (auto y = 0; y < mheight; ++y) {
                    ...
                    if (y < mheight-1 && m.wall(x, y, types::Direction::SOUTH)) {
                        // Find the central position in the thick maze.
                        const int cx = 2 * x;
                        const int cy = 2 * y + 1;
                        if (cx > 0) contents[cx-1][cy] = thickmaze::CellType::WALL;
                        contents[cx][cy] = thickmaze::CellType::WALL;
                        if (cx < twidth-1) contents[cx+1][cy] = thickmaze::CellType::WALL;
                    }

                    // Ignore the last row of maze when adding eastern walls.
                    if (x < mwidth - 1 && m.wall(x, y, types::Direction::EAST)) {
                        // Find the central position in the thick maze.
                        const int cx = 2 * x + 1;
                        const int cy = 2 * y;
                        if (cy > 0) contents[cx][cy-1] = thickmaze::CellType::WALL;
                        contents[cx][cy] = thickmaze::CellType::WALL;
                        if (cy < theight-1) contents[cx][cy+1] = thickmaze::CellType::WALL;
                    }
                }
    ...
    struct Homomorphism<maze::Maze, graphmaze::GraphMaze> {
        static const graphmaze::GraphMaze morph(const maze::Maze &m) {
            ...
            for (auto y=0; y < h; ++y)
                for (auto x=0; x < w; ++x) {
                    if (!m.wall(types::pos(x, y, types::Direction::EAST)))
                        vc.emplace_back(v, v + 1);
                    if (!m.wall(types::pos(x, y, types::Direction::SOUTH)))
